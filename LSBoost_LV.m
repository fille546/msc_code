...
beta_samples = rand(num_samples, 1) * diff (beta_range) + beta_range(1);
mu_samples = rand(num_samples, 1) * diff (mu_range) + mu_range(1);
alpha_samples = rand(num_samples, 1) * diff (alpha_range) + alpha_range(1);
...
tic;
european_approx_prices = arrayfun(@(idx) european_LV(S0_samples(idx), ...
    K_samples(idx), T_samples(idx), beta_samples(idx), mu_samples(idx), ...
    alpha_samples(idx)), 1:num_samples);
...
% Configure the LSBoost (MathWorks, n.d.-d). Use 10/100 learning cycles,
% and 1/0.1 learning rate.
tic;
ensemble_method1 = fitrensemble(X_train, Y_method1_train, 'Method', ...
    'LSBoost', 'NumLearningCycles', 100, 'Learners', 'Tree', ...
    'LearnRate',0.1);
M1_training = toc;
tic;
ensemble_method2 = fitrensemble(X_train, Y_method2_train, 'Method', ...
    'LSBoost', 'NumLearningCycles', 100, 'Learners', 'Tree', ...
    'LearnRate', 0.1);
M2_training = toc;
...
tic;
Y_pred_method1_val = predict(ensemble_method1, X_val);
M1_predict = toc;

% Use the validation data for predictions (M2).
tic;
Y_pred_method2_val = predict(ensemble_method2, X_val);
M2_predict = toc;
...
% MathWorks. (n.d.-d). Ensemble Algorithms. Retrieved December 10, 2023, 
% from https://se.mathworks.com/help/stats/ensemble-algorithms.html
